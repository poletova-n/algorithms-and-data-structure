...
int main()
{
  try {
    ...
    std::cout << text00 << ": "
              << (testBalanceBrackets(text00) ? "right" : "wrong") << '\n';

    const char* text01 = "( ) ok ";
    std::cout << text01 << ": "
              << (testBalanceBrackets(text01) ? "right" : "wrong") << '\n';

    const char* text02 = "( ( [] ) ) ok ";
    std::cout << text02 << ": "
              << (testBalanceBrackets(text02) ? "right" : "wrong") << '\n';

    const char* text03 = "( ( [ { } [ ] ( [ ] ) ] ) ) OK";
    std::cout << text03 << ": "
              << (testBalanceBrackets(text03) ? "right" : "wrong") << '\n';

    const char* text04 = "( ( [ { } [ ] ( [ ] ) ] ) ) ) extra right parenthesis ";
    std::cout << text04 << ": "
              << (testBalanceBrackets(text04) ? "right" : "wrong") << '\n';

    const char* text05 = "( ( [{ }[ ]([ ])] ) extra left parenthesis ";
    std::cout << text05 << ": "
              << (testBalanceBrackets(text05) ? "right" : "wrong") << '\n';

    const char* text06 = "( ( [{ ][ ]([ ])]) ) unpaired bracket ";
    std::cout << text06 << ": "
              << (testBalanceBrackets(text06) ? "right" : "wrong") << '\n';
