...
public:
	...
	BinarySearchTree<T> operator=(const BinarySearchTree&) = delete;
	BinarySearchTree<T> operator=(BinarySearchTree&&) = delete;
...
template<class T>
int BinarySearchTree<T>::count(Node* node) const
{
	if (node == nullptr)
	{
		return 0;
...
template<class T>
int BinarySearchTree<T>::heightTree(Node* node) const
{
	if (node == nullptr)
	{
		return 0;
