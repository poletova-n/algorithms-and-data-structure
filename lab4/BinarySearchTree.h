...
typename BinarySearchTree<T>::template Node<T> *BinarySearchTree<T>::iterativeSearchNode(const T &key) const
{
  Node<T> *head = root_;
  if (head == nullptr)
  {
...
{
  std::stack<Node<T> *> tempStack;
  std::stack<Node<T> *> thisTreeStack;
  std::stack<Node<T> *> srcTreeStack;
  ...
  std::cout << std::endl;
  if (thisTreeStack.size() != srcTreeStack.size())
    return false;
  while (thisTreeStack.size() != 0 && srcTreeStack.size() != 0)
  {
...
template<class T>
typename BinarySearchTree<T>::template Node<T> *BinarySearchTree<T>::searchMinNode(BinarySearchTree::Node<T> *node) const
{
  Node<T> *minNode = node;
