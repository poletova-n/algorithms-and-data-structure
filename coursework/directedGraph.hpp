...
private:
  ...
  void subDFS (bool *used, DirectedGraph::Node *node);
  void fillOrder(DirectedGraph::Node *node, bool used[], StackArray<int> &stack);
};

template<class T>
DirectedGraph<T>::DirectedGraph(int size):
  size_(size)
{
...
template<class T>
void DirectedGraph<T>::insertNode(T value)
{
  if (isEmpty())
  {
...
template<class T>
void DirectedGraph<T>::insertEdge(T node1, T node2)
{
  if (isEmpty())
  {
    throw GraphEmpty();
...
template<class T>
void DirectedGraph<T>::deleteEdge(T node1, T node2)
{
  if (isEmpty())
  {
    throw GraphEmpty();
...
template<class T>
void DirectedGraph<T>::subDFS(bool *used, DirectedGraph::Node *node)
{
...
void DirectedGraph<T>::SCC()
{
  ...
  for (int i = 0; i < size_; i++)
  {
    if (!used[i])
    {
      fillOrder(searchNodeByIndex(i), used, stack);
    }
  }
  DirectedGraph<T> transposedGraph = getTranspose();
  for (int i = 0; i < size_; i++)
  {
    used[i] = false;
  }
  while (!stack.isEmpty())
  {
    ...
    if (!used[v])
    {
      transposedGraph.subDFS(used, searchNodeByIndex(v));
      std::cout << "\n";
    }
  }
}
...
template<class T>
DirectedGraph<T> DirectedGraph<T>::getTranspose()
{
  DirectedGraph<T> transposedGr;
  std::vector<DoubleLinkedList> used;
  for (int i = 0; i < size_; i++)
  {
    DoubleLinkedList temp;
    used.push_back(temp);
    transposedGr.insertNode(searchNodeByIndex(i)->value_);
  }
  Node *current = head_;
  while (current)
  {
    ...
    for (int i = 0; i < size_; i++)
    {
      if (!used[i].searchItem(current->number_))
      {
        if (allEdges_[current->number_].searchItem(i))
        {
          temp.insertTail(i);
          used[current->number_] = temp;
          transposedGr.insertEdge(searchNodeByIndex(i)->value_, current->value_);
        }
      }
    }
    current = current->next_;
...
template<class T>
void DirectedGraph<T>::fillOrder(DirectedGraph::Node *node, bool used[], StackArray<int> &stack)
{
  used[node->number_] = true;
  DoubleLinkedList nodeWays = allEdges_[node->number_];
  for (int i = 0; i < nodeWays.count(); i++)
  {
    int index = nodeWays.getItem(i);
    if (!used[index])
    {
      fillOrder(searchNodeByIndex(index), used, stack);
    }
  }
  stack.push(node->number_);
}

template<class T>
typename DirectedGraph<T>::Node *DirectedGraph<T>::searchNodeByValue(T value)
{
  Node *temp = head_;
...
template<class T>
typename DirectedGraph<T>::Node *DirectedGraph<T>::searchNodeByIndex(int index)
{
  Node *temp = head_;
...
template<class T>
bool DirectedGraph<T>::includeNode(T value)
{
  Node *current = head_;
  while (current->next_ != nullptr)
  {
...
template<class T>
bool DirectedGraph<T>::includeEdge(T node1, T node2)
{
