...
public:
  ...
  T* iterativeWidthWalk();

  bool isSimilar(BinarySearchTree<T>& obj);
...
template<class T>
T* BinarySearchTree<T>::iterativeWidthWalk()
{
  size_t size = getCount();
  size_t i = 0;
  T* arr = new T[size];
  QueueArray<Node*> queue(size);
  queue.push(root_);
  Node* node = nullptr;
...
template<class T>
bool BinarySearchTree<T>::isSimilar(BinarySearchTree<T>& obj)
{
  size_t size = getCount();
  if (size != obj.getCount())
	return false;
  T* arr1 = iterativeInorderWalk();
  T* arr2 = obj.iterativeInorderWalk();
