...
public:
	...
	// 10 Обход дерева по ширине
	void floorWalk() const;
	// 11 Проверка деревьев на схожесть
	bool isSimilar(const BinarySearchTree<T>& src);
};
...
T* BinarySearchTree<T>::iterativeInorderWalk() const
{
	...
	Node<T>* temp;
	temp = root_;
	StackArray<Node<T>*> treeStack(this->getCount());
	T* tempArray = new T[this->getCount()];
	while (temp != nullptr || !treeStack.isEmpty())
	{
		if (temp != nullptr)
		{
			treeStack.push(temp);
...
template<class T>
void BinarySearchTree<T>::floorWalk() const
{
	if (!this->root_)
	{
		std::cout << "Tree is empty\n";
	...
	QueueArray<Node<T>*> tempQueue((this->getCount()) + 1);
...
bool BinarySearchTree<T>::isSimilar(const BinarySearchTree<T>& src)
{
	...
	std::cout << "First tree iterativeStackWalk:  ";
	T* tempArray1 = this->iterativeInorderWalk();
	std::cout << "Second tree iterativeStackWalk:  ";
	T* tempArray2 = src.iterativeInorderWalk();
